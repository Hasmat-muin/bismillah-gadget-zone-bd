...
// 🎯 সেভ করা সময়ক্রম (createdAt) অনুযায়ী সোর্ট করে বিলবোর্ড রেন্ডার করা
    ...
    try {
        ...
        sortedItems.forEach((item, index) => {
            ...
            let mediaHTML = isVideo ? 
                `<video src="${srcUrl}" autoplay loop muted playsinline webkit-playsinline preload="auto"></video>` : 
                `<img src="${srcUrl}" alt="Billboard Banner">`;
...
function startBillboardAutoSlide(track, totalSlides) {
    ...
    billboardAutoSlideInterval = setInterval(slideNext, 3000);

    track.addEventListener('mouseenter', () => clearInterval(billboardAutoSlideInterval));
    track.addEventListener('mouseleave', () => {
        if (billboardAutoSlideInterval) clearInterval(billboardAutoSlideInterval);
        billboardAutoSlideInterval = setInterval(slideNext, 3000);
    });

    track.addEventListener('touchstart', () => clearInterval(billboardAutoSlideInterval), { passive: true });
