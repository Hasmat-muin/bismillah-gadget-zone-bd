...
// 🛒 localStorage থেকে আগের কার্ট লোড করা হচ্ছে (থাকলে), না থাকলে খালি অ্যারে
let cart = JSON.parse(localStorage.getItem('bg_cart')) || [];
let allProductsData = {}; 
let selectedVariantsGlobal = {}; 
let activeCategories = []; 

// 🛒 কার্ট পরিবর্তন হলেই সাথে সাথে localStorage-এ সেভ করে দেয়
        ...
        let categoryHTML = `
            <div class="category-section" id="sec-${safeId}">
                <div class="category-header">
                    ...
                    <span class="see-more-link" onclick="viewFullCategory('${category.replace(/'/g, "\\'")}')">আরও দেখুন ➜</span>
                </div>
...
function switchCategory(categoryName, element) {
    // full-view মোডে থাকলে আগে সেটা বন্ধ করে স্বাভাবিক ভিউতে ফেরত আনো
...
// 👀 "আরও দেখুন" (See More) ক্লিক করলে ওই ক্যাটাগরির সব প্রোডাক্ট উপর থেকে নিচে (vertical) স্ক্রল ভিউতে দেখাবে
function viewFullCategory(categoryName) {
    const safeId = categoryName.replace(/\s+/g, '-');

    // শুধু সিলেক্টেড ক্যাটাগরির সেকশন দেখাও, বাকিগুলো লুকাও
    document.querySelectorAll('.category-section').forEach(sec => {
        sec.style.display = (sec.id === `sec-${safeId}`) ? 'block' : 'none';
    });

    // সব গ্রিড থেকে full-view ক্লাস সরিয়ে শুধু টার্গেট গ্রিডে বসাও
    document.querySelectorAll('.products-grid').forEach(grid => grid.classList.remove('full-view'));
    const targetGrid = document.getElementById(`grid-${safeId}`);
    if (targetGrid) targetGrid.classList.add('full-view');

    // ক্যাটাগরি ট্যাব বার লুকিয়ে "সব ক্যাটাগরি দেখুন" বাটন দেখাও
...
// ⬅ "সব ক্যাটাগরি দেখুন" ক্লিক করলে আগের হোম ভিউতে ফিরে যাবে
function exitFullCategoryView() {
    ...
    // "All Items" ট্যাব-কে আবার একটিভ করো
...
function searchProducts(query) {
    ...
    // সার্চ শুরু হলে ফুল-ভিউ মোড থেকে বের করে আনো
    const backWrap = document.getElementById('back-to-all-wrap');
    if (backWrap && backWrap.style.display !== 'none') {
        document.querySelectorAll('.products-grid').forEach(grid => grid.classList.remove('full-view'));
...
document.addEventListener("DOMContentLoaded", () => {
    updateCartUI(); // পেজ লোড হওয়া মাত্র সেভ করা কার্ট আইটেম ও কাউন্ট দেখানো হচ্ছে
